...
public class TJingpaiDAO extends HibernateDaoSupport
{
	...
	public static final String JINGPAI_GOODS_ID = "jingpaiObjfwId";
	...
	public TJingpai findById(java.lang.Integer id){
		...
		try{
			TJingpai instance = (TJingpai) getHibernateTemplate().get("com.model.TJingpai", id);
			return instance;
		} catch (RuntimeException re){
	...
	public List findByProperty2(int value){

		
		
//		String sql="";
//		sql="from TObjfw where objfwDel='no' and objfwName like '%"+objfwName.trim()+"%'"+" order by objfwCatelogId";
//		
//		List objfwList=objfwDAO.getHibernateTemplate().find(sql);
//		request.put("objfwList", objfwList);		
		
			String queryString = "from TJingpai where jingpaiObjfwId="+value;
			 List objfwList=getHibernateTemplate().find(queryString);
	...
	public List findByJingpaiObjfwId(Object jingpaiObjfwId)
	{
		return findByProperty(JINGPAI_GOODS_ID, jingpaiObjfwId);
	}
